(function() {
    ...
    // The child message was changed by our cloud function replace the message if it's already rendered
    messages.limitToLast(MESSAGE_COUNT).on('child_changed', function(data){
        var val = data.val();
        var existingMessage = document.getElementById(data.key);
        if (existingMessage) {
            existingMessage.innerHTML = messageTemplate(data.key, val.user, val.text).innerHTML;
        }
    });
